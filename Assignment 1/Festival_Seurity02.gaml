...
species InfoCentre parent: Building {
	...
	{
		ask Guest at_distance InfoCentreSize
		{
			if(self.ifGuestBad)
			{
				Guest ifGuestBad <- self;
				ask FestSecurity
				{
					if(!(self.targets contains ifGuestBad))
					{
						self.targets <+ ifGuestBad;
						write 'InfoCenter found a bad guest (' + ifGuestBad.name + '), sending Security after it';	
					}
				}
			}
		}
...
species FestSecurity skills: [moving]
{
	list<Guest> targets;
	aspect default
	{
	...
	reflex findBadGuest when: length(targets) > 0
	{
		if(dead(targets[0]))
		{
		...
		{
			do goto target:(targets[0].location) speed: speedPolice;
		}
	}
