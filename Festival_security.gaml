...
species Guest skills:[moving]{
	...
	reflex askGuest when: target!=nil and target.location = InfoCentreLoc and fetchGuard = false {
		string msg <- name; 
	...
	reflex askInfoCentre when: target!=nil and target.location = InfoCentreLoc and location distance_to(target.location) < InfoCentreSize 
	{
		if (!fetchGuard){
			...
			ask InfoCentre {
				myself.target <- self.Security; 
				write "TARGET SECURITY" + myself.target;
			}
		}
	}
	...
	reflex findBadGuest
	{
		ask Guest at_distance(5) {
			if(bad) { 
				if !(myself.badGuest contains(self)){
					add item: self to: myself.badGuest;
				}
				myself.target <- one_of(InfoCentre);
				myself.color <- #orange; 
				myself.food <- nil; 
				myself.fetchGuard <- true; 
				write "BAD GUEST DISCOVERED";
			}
		}
	}
}
...
species InfoCentre parent: Building {
	...
	list<FoodStore> FoodStores; 
	list<DrinkStore> DrinkStores;
	SecurityGuard Security <- one_of(SecurityGuard);
...
species SecurityGuard skills:[moving] parent: Building {
	float size <- 1.0;
	rgb color <- #violet; 
